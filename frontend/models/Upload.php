...
class Upload
{
	...
	public static function fileInput($model, $attr, $image = false, $multiple = false, $redirect = false){
		...
		if($multiple){
			...
			$view = '@frontend/views/upload/main-journal';
		}
	...
	public static function showFile($model, $attr, $controller, $action = 'download'){
		...
		if($model->{$db_file}){
			...
			if($ext == 'pdf'){
				...
				$link = Url::to([$controller . '/' . $action, 'attr' => $attr, 'id' => $model->id]);
			}
			
			return '<a href="'. Url::to([$controller . '/' . $action, 'attr' => $attr, 'id' => $model->id]) . '" target="_blank">
