...
class SubmissionController extends Controller
{
    ...
    {
		$query =  Article::find()->where(['user_id' => Yii::$app->user->identity->id])->orderBy('status ASC');
	...
	public function actionPaymentSubmit($id)
    {
		$model = $this->findModel($id);
		$model->scenario = WorkflowScenario::enterStatus('bo-payment-submit');
		
		$ori_status = $model->status;
		if ($model->load(Yii::$app->request->post())) {
			
        $model->payment_submit_at = new Expression('NOW()');
		
		$model->sendToStatus('bo-payment-submit');
		
        if($model->save()){
			$model->sendEmail();
            Yii::$app->session->addFlash('success', "The payment has been successfully submitted.");
        ...
        return $this->render('payment_submit', [
            'model' => $model,
        ]);
    }
    ...
    {
        $model = new Article();
		$model->journal_id = Yii::$app->params['journal_id'];
    ...
    public function actionUploadArticle($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
			...
			if($wfaction == 'btn-submit'){
				...
				/* PRE-EVALUATE */
				$model->yearly_number = $model->genYearlyNumber();
