...
<a href="#" id="remove_<?=$unique?>" class="btn btn-danger" data-type="DELETE" data-url="<?=Url::to([$model->file_controller . '/delete-file', 'attr' => $attr_name, 'id' => $model->id])?>" title="Delete"><span class="fas fa-trash"></span></a>
  ...
  var req = $.ajax({
	...
	success: function(result){
		if(result.good == 1){
			...
			$('#btn_$unique ').show();
			$('#img_$unique ').html('');
			link.html('<span class=\"fas fa-trash\"></span>');
